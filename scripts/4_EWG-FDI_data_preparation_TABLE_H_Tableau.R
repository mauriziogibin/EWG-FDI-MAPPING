...
#- Clear workspace
rm(list=ls())
#cDIR = '/home/maurizio/work/EWG-FDI-22-10' # LINUX
cDIR = 'E:/EWG-FDI-22-10/'
...
# for(i in fnames){
setwd('./original/')
fList <- list.files(path='.',pattern=glob2rx('*table*h*.csv'))
...
sum(fdi[year==2019,]$totwghtlandg)
sum(fdi.tableau[year==2019,]$totwghtlandg)
sum(fdi$totvallandg,na.rm = T)
sum(fdi.tableau$totvallandg,na.rm = T)
fdi <- NULL;gc()
